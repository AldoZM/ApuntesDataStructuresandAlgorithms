...
CHAINING (lista por bucket):        OPEN ADDR. linear probing:
  [0] → NULL                          [0] │
  [1] → NULL                          [1] │
  [2] → (A) → (B) → NULL              [2] │ A     hash 2 libre
  [3] → (C) → NULL                    [3] │ B     C quería 3 → ocupado por B?
  [4] → NULL                          [4] │ C     no: B fue a 3 (2 lleno),
...
REHASH (load factor α = n/m > 0.75):  m: 8 → 16, reinsertar todo
  cada clave se recoloca con  hash % nueva_capacidad
\end{verbatim}
\end{dsacomofunciona}
